...
image:wildfly-intro.png[align="center"]
...
<5> Call HTTP GET, specifies the type of resource
...
----
<1> Creates the destination resource during deployment
...
-> (indent) _8080_ for applications with JNDI and EJB multiplexed +
-> (indent) _9990_ for management, for both HTTP/JON & Native API +
-> Only overhead is the initial HTTP Upgrade request/response

[NOTE]
[role="speaker"]
====
* Basically to support high-density environments (like OpenShift) where physical resources are shared and scarce - another example of why JBoss is designed for the cloud / PaaS. The better examples are small memory footprint and fast startup (delivered in AS 7).
...
-> Management model consists of _Users_, _Roles_, _Permissions_ +
...
-> (indent) Not a Red Hat JBoss Operations Network replacement
====
